...
from prunning.utils.FilterPrunner import FilterPrunner
from prunning.utils.prune_mtcnn import prune_mtcnn
from prunning.utils.util import ListDataset, printProgressBar, total_num_filters
...
def test(model, path):
    ...
    for i_batch, sample_batched in enumerate(dataloader):
        ...
        with torch.set_grad_enabled(False):
            pred_label, pred_offsets, pred_landmark = model(input_images)
...
def get_args():
    ...
    parser.add_argument("--train_path", type = str, default = "./annotations/imglist_anno_48_train.txt")
    parser.add_argument("--test_path", type = str, default = "./annotations/imglist_anno_48_val.txt")
...
if __name__ == '__main__':
    ...
    model = ONet(is_train=True, train_landmarks=True).to(device)
    model.load_state_dict(torch.load("pretrained_weights/mtcnn/best_onet_landmark.pth", map_location=lambda storage, loc: storage))
    
    prunner = FilterPrunner(model, use_cuda = True) 
    
    save_dir = './prunning/saving_onet_prunning_result'
    ...
    for it in range(iterations):
        ...
        torch.onnx.export(model,  torch.randn(1, 3, 24, 24).to(device),
                './onnx2ncnn/pruned_onet_{}.onnx'.format(it),
