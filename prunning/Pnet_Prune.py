...
def get_args():
    ...
    parser.add_argument("--train_path", type = str, default = "./annotations/imglist_anno_12_train.txt")
    parser.add_argument("--test_path", type = str, default = "./annotations/imglist_anno_12_val.txt")
...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load("./pretrained_weights/mtcnn/best_pnet.pth", map_location=lambda storage, loc: storage))
    
    prunner = FilterPrunner(model, use_cuda = True) 
    
    save_dir = './prunning/saving_pnet_prunning_result'
    ...
    for it in range(iterations):
        ...
        torch.onnx.export(model, torch.randn(1, 3, 12, 12).to(device), 
                './onnx2ncnn/pruned_pnet_{}.onnx'.format(it), 
